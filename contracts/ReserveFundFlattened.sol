...
contract ReserveFund is Fund {
    ...
    uint public constant creationTime = 1543536000; //30.11.2018
    uint public totalReserved = address(this).balance;
    uint public firstLimitPart = totalReserved.div(2).mul(10 ** 4);
    uint public secondLimitPart = totalReserved.div(2).mul(10 ** 4);
  /**
   * @dev Constructor that sets the initial contract parameters
   * @param _token ERC223 address of the ERC-223 token
   */
  ...
  function _timeLimit(uint _value) internal {
    if (block.timestamp < creationTime.add(90 days)) {
      require(_value <= firstLimitPart);
      firstLimitPart = firstLimitPart.sub(_value);
    } else if (
      block.timestamp >= creationTime.add(90 days) &&
      block.timestamp < creationTime.add(180 days)
    ) {
      require(_value <= firstLimitPart.add(secondLimitPart));
      if (firstLimitPart >= _value) {
        firstLimitPart = firstLimitPart.sub(_value);
      } else {
        secondLimitPart = secondLimitPart.sub(_value);
      }
    } 
  }
